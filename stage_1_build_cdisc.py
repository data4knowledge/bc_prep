...
def process_instances(base_uri, ns_uri, ra_uri):
  ct_server = CTService()
  files = files_in_dir('converted_data')
          ...
          for data_type in item["data_type"]: 
            ...
            if "value_set" in data_type:
              ...
              for term in data_type["value_set"]: 
                ...
                print(result[2]['parent']['uri'])
                print(result[2]['child']['uri'])
                if result == None:
                ...
                record = {
                  ...
                  "cl": cl,
                  "cli": cli,
                  "term_uri": result['child']['uri'],
                  "notation": result['child']['notation'],
                  "pref_label": result['child']['pref_label']
                }
...
delete_dir("load_cdisc_data")

ns_s_json = RaService().namespace_by_name("CDISC BC namespace")
#print(ns_s_json)
...
for k, v in nodes.items():
  csv_filename = "load_cdisc_data/node-%s-1.csv" % (snakecase(k))
  write_nodes(v, csv_filename)

for k, v in relationships.items():
  csv_filename = "load_cdisc_data/relationship-%s-1.csv" % (k.lower())
